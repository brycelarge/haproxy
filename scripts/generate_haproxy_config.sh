...
"
    ...
    done < <(echo "$JSON_CONFIG" | jq -r '.domain_mappings[] | select(.frontend == "https-offloading" and .frontend != "https-offloading-ip-protection") | "\(.domains[]//\(.domain)) \(.backend)"')
...
generate_https_offloading_ip_protection_frontend_config() {
    ...
    done < <(echo "$JSON_CONFIG" | jq -r '.domain_mappings[] | select(.frontend == "https-offloading-ip-protection") | "\(.domains[]//\(.domain)) \(.backend)"')
